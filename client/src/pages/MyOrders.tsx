...
export default function MyOrders() {
  ...
  return <main className="orders-tracking-page" dir="rtl"><section className="container orders-tracking-wrap"><div className="orders-tracking-intro"><span className="eyebrow"><ClipboardList size={14} /> خدمة ما بعد الطلب</span><h1>تابعي <i>طلباتك</i> بسهولة</h1><p>اكتبي رقم الموبايل المستخدم عند الطلب لعرض حالة الشحن والمنتجات والإجمالي. لا تظهر في هذه الصفحة بيانات العنوان أو أي تفاصيل خاصة.</p></div><form className="tracking-form" onSubmit={submit}><label htmlFor="tracking-phone">رقم الموبايل المستخدم عند الطلب</label><div><Input id="tracking-phone" inputMode="tel" autoComplete="tel" value={phone} onChange={event => setPhone(event.target.value)} placeholder="مثال: 01007891081" /><Button type="submit" disabled={loading}>{loading ? <Loader2 className="animate-spin" size={17} /> : <Search size={17} />} بحث عن طلباتي</Button></div>{error && <p className="tracking-error">{error}</p>}<small><ShieldCheck size={13} /> نعرض ملخص الطلب فقط للمحافظة على خصوصية بياناتك.</small></form>{searched && !loading && (orders.length ? <section className="tracking-results" aria-live="polite">{orders.map(order => <article className="tracking-card" key={order.order_id}><div className="tracking-card-head"><div><span>طلب #{order.order_id.slice(0, 8).toUpperCase()}</span><time>{new Date(order.created_at).toLocaleString("ar-EG", { dateStyle: "medium", timeStyle: "short" })}</time></div><b className={`tracking-status status-${order.status}`}>{trackingStatusLabels[order.status] ?? order.status}</b></div><div className="tracking-items">{order.items.map((item, index) => <div className="tracking-line" key={`${item.product_title}-${index}`}><div className="tracking-product-mark">ع</div><div><b>{item.product_title}</b><span>{item.size !== "N/A" ? `مقاس ${item.size}` : "بدون مقاس"} · {item.color} · {item.quantity} قطعة</span></div><strong>{(item.unit_price * item.quantity).toLocaleString("ar-EG")} ج.م</strong></div>)}</div><div className="tracking-total"><span>الدفع عند الاستلام</span><b>{Number(order.total_amount).toLocaleString("ar-EG")} ج.م</b></div></article>)}</section> : <section className="tracking-empty"><PackageCheck size={31} /><h2>لا توجد طلبات بهذا الرقم</h2><p>تأكدي من رقم الموبايل الذي أُدخل عند إتمام الطلب، أو تواصلي معنا عبر واتساب للمساعدة.</p></section>) }<Link href="/" className="tracking-back"><ArrowLeft size={16} /> العودة للتسوق</Link></section></main>;
}
