...
const blankProduct = () => ({ title: "", description: "", category: "men" as Category, price: "", salePrice: "", isActive: true, variants: [blankVariant()], imageUrl: "", files: [] as File[], existingImages: [] as ProductImage[] });
...
export default function AdminDashboard() {
  ...
  async function save(event: React.FormEvent) {
    ...
    const variants = form.variants.map(item => ({ size: item.size.trim() || "N/A", color: item.color.trim(), stock_quantity: Number(item.stock_quantity) })).filter(item => item.color && item.stock_quantity >= 0);
    if (!form.title.trim() || !form.description.trim() || !form.price || !variants.length) return toast.error("أكمل اسم ووصف وسعر المنتج وخياراً واحداً على الأقل للمقاس واللون والمخزون.");
    setSaving(true);
    ...
    const clearVariants = await supabase.from("product_variants").delete().eq("product_id", savedProductId);
    if (clearVariants.error) { setSaving(false); return toast.error("تعذّر تحديث خيارات المخزون."); }
    const variantResult = await supabase.from("product_variants").insert(variants.map(item => ({ ...item, product_id: savedProductId })));
